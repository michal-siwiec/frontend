...
describe('useFetchUrl', () => {
  ...
  it('calls getSignedUrl with correct bucket and key and returns the URL', async () => {
    ...
    const { result } = renderHook(() =>
      useFetchUrl({ bucket: 'budoman-production', key: 'images/products/foundation_materials/powłoka_przeciwwilgociowa.jpeg' })
    );
  ...
  it('calls getSignedUrl with default bucket, key and returns the URL', async () => {
    ...
    const { result } = renderHook(() =>
      useFetchUrl({ key: 'images/products/foundation_materials/powłoka_przeciwwilgociowa.jpeg' })
    );
